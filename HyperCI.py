...
def compute_loss(Y_true, treatments, results, idx_trn, idx_select):
    # 提取真实潜在结果
    y1_true = Y_true[1]  # 处理组的真实潜在结果
    y0_true = Y_true[0]  # 对照组的真实潜在结果
    rep = results['rep']  # 模型生成的隐藏表示（用于计算 Wasserstein 距离）
    y1_pred = results['y1_pred']  # 模型预测的处理组结果
    y0_pred = results['y0_pred']  # 模型预测的对照组结果

    # 根据处理变量选择最终预测结果
    yf_pred = torch.where(treatments > 0, y1_pred, y0_pred)  # 如果样本属于处理组，则选择 y1_pred，否则选择 y0_pred

    # 平衡处理组和对照组的样本数量（用于 Wasserstein 距离计算）
    num_balance_max = 2000  # 平衡样本的最大数量
    idx_balance = idx_select if len(idx_select) < num_balance_max else idx_select[: num_balance_max]
    # 根据处理变量划分为处理组和对照组的隐藏表示
    rep_t1 = rep[idx_balance][(treatments[idx_balance] > 0).nonzero()]  # 处理组的隐藏表示
    rep_t0 = rep[idx_balance][(treatments[idx_balance] < 1).nonzero()]  # 对照组的隐藏表示

    # 计算处理组和对照组隐藏表示之间的 Wasserstein 距离
    dist, _ = utils.wasserstein(rep_t1, rep_t0, device, cuda=True)

    # 根据处理变量选择每个样本的真实潜在结果
    YF = torch.where(treatments > 0, y1_true, y0_true)  # 如果样本属于处理组，则选择 y1_true，否则选择 y0_true

    # 对潜在结果进行标准化（如果启用）
    if args.normy:
        ym = torch.mean(YF[idx_trn])  # 训练集中潜在结果的均值
        ys = torch.std(YF[idx_trn])  # 训练集中潜在结果的标准差
        YF_select = (YF[idx_select] - ym) / ys  # 标准化后的潜在结果
    else:
        YF_select = YF[idx_select]  # 不进行标准化，直接使用潜在结果

    # 计算预测误差（均方误差）
    loss_mse = torch.nn.MSELoss()  # 定义均方误差损失函数
    loss_y = loss_mse(yf_pred[idx_select], YF_select)  # 计算预测值和真实值之间的误差

    # 计算总损失：预测误差 + Wasserstein 距离的正则化项
    ...
    loss_result = {
        'loss': loss,       # 总损失
        'loss_y': loss_y,   # 预测误差
        'loss_b': dist      # Wasserstein 距离
    }

    return loss_result  # 返回包含各项损失的字典
...
def load_data(dataset, path, num_exp=10, graph_type='hypergraph', index_type='hyper_index', hyper_form_type='processed'):
    ...
    for i in range(num_exp):# 划分训练、验证和测试集
